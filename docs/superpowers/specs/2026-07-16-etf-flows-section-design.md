...
- **⚠️ KRX rate-limit — 라이브 리허설(2026-07-16)에서 확정**: KRX는 **세션당 ~200 데이터 요청**만 허용하고 이후 전부 차단(에러). 개별 ETF 1,146개 전량 순회는 **원천적으로 불가**(20콜 프로브의 외삽이 틀렸음 — 리허설이 뒤집음). 과도 호출 시 IP 단위 로그인까지 일시 차단되어 전종목 수급·publisher까지 오염.
  → **거래대금 상위 N개(N=120)로 제한**(§4.1). 소형 ETF는 순매수 0 근처라 TOP5 미진입 → 커버리지 손실 없음. 대형 ETF(KODEX 200 외국인 1,793억 등)는 거래대금 상위라 반드시 포함.
...
- 개별 ETF 루프 — **거래대금 상위 N개(기본 120)만**(KRX rate-limit 회피): `get_etf_price_change_by_ticker(from, to)`(1콜)로
  거래대금 내림차순 상위 N 티커 선정 + `get_etf_ticker_list(todate)`(이름 캐시 워밍) → 각 티커 `get_etf_trading_volume_and_value(from, to, ticker)`,
  `기관합계`·`외국인`의 `(거래대금, 순매수)` 추출 / 1e8. 랭킹 콜 실패 시 이름목록 상위 N 폴백.
  세션 총 KRX 콜 ≈ 전종목 4 + ETF 집계·랭킹·워밍 3 + N(120) ≈ 127 < 200(안전).
- **best-effort**:
...
- Excel ETF 시트, ETF 섹터/테마 분류.
- ETN 수급, ETF에 대한 LLM 해설.
- (유동성 필터는 rate-limit 대응으로 §4.1에서 core 채택 — future 아님.)
